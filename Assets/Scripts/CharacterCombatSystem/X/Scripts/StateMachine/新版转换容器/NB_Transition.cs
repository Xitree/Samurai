...
public class NB_Transition : ScriptableObject
{
    ...
    private class StateAcitionConfig 
    {
        ...
        public void Init(StateMachineSystem stateMachineSystem) {
            ...
            foreach (var condition in conditions) {
                condition.InitCondition(stateMachineSystem);
            }
        }
    }
    ...
    public void Init(StateMachineSystem stateMachineSystem) 
    {
        this._stateMachineSystem = stateMachineSystem;
    ...
    private void SaveAllStateTransitionInfo() 
    {
        foreach (var item in configStateData)
        {
            ...
            if (!states.ContainsKey(item.fromState)) 
            {
                ...
                // UtilitiesTools.DebugPrint("当前不存在key，创建Key成功，初始化容器");
                states[item.fromState].Add(item);
                // UtilitiesTools.DebugPrint("创建Key成功，添加条件成功");
            }
            else 
            {
                states[item.fromState].Add(item);
                // UtilitiesTools.DebugPrint("当前存在key，添加条件成功");
            }
    ...
    public void TryGetApplyCondition()
    {
        ...
        //遍历当前状态能转的状态是否有条件成立
        if (states.ContainsKey(_stateMachineSystem.currentState)) 
        {
            foreach (var stateItem in states[_stateMachineSystem.currentState])
            {
                foreach (var conditionItem in stateItem.conditions)
                {
                    if (conditionItem.ConditionSetUp())
                    {
                        if (conditionItem.GetConditionPriority() >= conditionPriority)
                        {
                            ...
                            // UtilitiesTools.DebugPrint("当前条件优先级" + conditionItem.GetConditionPriority());
                        }
                    }
                }
            }
        ...
        if(toStates.Count!=0 || toStates != null) 
        {
            ...
            foreach (var item in toStates)
            {
                if (item.GetStatePriority() >= statePriority)
                {
                    ...
                    // UtilitiesTools.DebugPrint("状态优先级" + item.GetStatePriority());
                }
            }
        }
        ...
        if (toState != null) 
        {
            _stateMachineSystem.currentState.OnExit();
            _stateMachineSystem.currentState = toState;
            _stateMachineSystem.currentState.OnEnter();            
            toStates.Clear();
            conditionPriority = 0;
            statePriority = 0;
            toState = null;
